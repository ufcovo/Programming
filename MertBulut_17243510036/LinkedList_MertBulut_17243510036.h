...
void LinkedList<T>::insert_descending(T& new_item) {
	...
	Node <T> *ptr = new Node <T>;			//creates a pointer node which is ptr
	Node <T> *temp = new Node <T>; 		   //creates a temp node to assign the bigger value of descen_order
	
	Node <T> *newNode = new Node <T>;	 //create a new node which is type of Node
	newNode -> data = new_item;  		//stores new_item in the newNode of data
	newNode -> next = NULL;			   //stores NULL in the newNode of next
	
	ptr = head;			//stores head in the ptr which is pointer type
	
	if (head == NULL)
	{
		last = newNode;
		head = last;				//instead of 2 lines, we can use head = last = newNode;
	}
