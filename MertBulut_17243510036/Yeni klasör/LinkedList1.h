...
void LinkedList<T>::insert_descending(T& new_item) {
	...
	newNode -> data = new_item;  		//stores new_item in the new node
	newNode -> next = NULL;
	Node <T> *p = new Node <T>;			//creates a pointer node
	Node <T> *temp = new Node <T>; 		//creates a temp node to carry the bigger value
